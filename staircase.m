...
%                     sc.step=exp(log(sc.step)*nR/(1+nR));
                    sc.step = sc.step; % fixed log step size 
                end
...
%             end
            if ok, val=10^(log10(val)-sc.step(1)); % decrease
            else val = 10^(log10(val)+sc.step(2)); % increase
            end
        else
            if ok, val=val-sc.step(1);
            else val=val+sc.step(2);
...
elseif strcmpi(cmd,'create')
    ...
    if sc.logStep
        sc.step(2)=exp(log10(sc.step(1))*(1/(1-prob^sc.upDown(2))-1));
    else
...
%     figure(7);
    ...
    for i=1:nsc
        subplot(nsc,1,i);
%         tempdata = sc(i).data(:,1).*100;
        semilogy(sc(i).data(:,1),'b'); hold all;
        ind=find(sc(i).data(:,2)); % index for correct trials
        h(1)=semilogy(ind,sc(i).data(ind,1),'.g','markersize',16);
        ind=find(sc(i).data(:,2)==0); % for wrong trials
        h(2)=semilogy(ind,sc(i).data(ind,1),'.r','markersize',16);
        ...
        x=xlim;
        h(3)=semilogy(iR,sc(i).reversal,'+k','markersize',8);
        h(4)=line(x,[1 1]*sc(i).threshold(1),'color','m','LineStyle',':');
        
%         axis([0 length(sc(i).data) 0 0.1]); % commented out by WP
